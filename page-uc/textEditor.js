    ...
    switch (action) {
      ...
      case "keepOnlyNumbers":
        ...
        function numberToWords(num) {
          ...
          return num.toString(); // For numbers >= 1,000,000, return as is
        }
        ...
        function wordsToNumber(word) {
          ...
          return wordToNum[word] !== undefined ? wordToNum[word] : word;
        }

      case "numbersToEnglishWords":
        textArea.value = textArea.value.replace(/\d+/g, (match) => {
          return numberToWords(parseInt(match));
        });
        break;

      case "englishWordsToNumbers":
        textArea.value = textArea.value.replace(
          /\b(zero|one|two|three|four|five|six|seven|eight|nine|ten)\b/gi,
          (match) => {
            return wordsToNumber(match.toLowerCase());
          }
        );
        break;
