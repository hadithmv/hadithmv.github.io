...
// Book codes (sourceBook from 04-registry-quranColumns.csv)
...
export function loadSurahNames() {
  ...
  return fetchCSV("../data/05-registry-quranSurahs.csv").then(
    function (rows) {
...
export function loadColumnRegistry() {
  ...
  return fetchCSV("../data/04-registry-quranColumns.csv").then(
    function (rows) {
      if (rows.length === 0) return [];
