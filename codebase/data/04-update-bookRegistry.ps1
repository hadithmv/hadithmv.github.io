...
#     files as ANSI, which misparses multi-byte characters into "Missing
#     closing '}'" errors. The whole file is deliberately plain ASCII so it
#     renders in any console anywhere; keep both the BOM and the ASCII.
...
function Write-Add($text) { Write-Host "  [OK] $text" -ForegroundColor Green }
function Write-Rename($text) { Write-Host "  [REN] $text" -ForegroundColor Magenta }
function Write-Skip($text) { Write-Host "  [SKIP] $text" -ForegroundColor DarkGray }
function Write-Info($text) { Write-Host "    $text" -ForegroundColor Gray }

Write-Host "`nHadithmv - Update Book Metadata" -ForegroundColor White
...
foreach ($row in $rows) {
    ...
    if (-not (Test-Path $csvFile) -and $virtualBooks -notcontains $code) {
        Write-Warning "  [WARN] $code - registered but CSV file missing"
        $missing++
    }
}
...
if ($added -gt 0) { Write-Host "  [OK] $added added" -ForegroundColor Green }
if ($renamed -gt 0) { Write-Host "  [REN] $renamed files renamed" -ForegroundColor Magenta }
if ($missing -gt 0) { Write-Host "  [WARN] $missing missing CSV files" -ForegroundColor Red }
if ($normalized -gt 0) { Write-Host "  [OK] $normalized files normalized to LF" -ForegroundColor Green }
...
try {
    ...
    Write-Host "  [FAIL] manifest refresh failed: $_" -ForegroundColor Red
    exit 1
}
