 ...
 * info modal (src/js/book-info.js) can export its pane with the exact same
...
// subtitle, a hairline, then the brand/version/URL (a live link — clickable
// in Word, the HTML Book and the EPUB cover). The info modal adds its kind
// line ("Biography of the author" …) before the name pair and the fact
// strip after the URL — each group separated by a hairline.
...
function titlePageHTML(cfg, meta, siteURL, versionText) {
  ...
  if (cfg.kindTitle)
    h += '<p class="kind">' + cfg.kindTitle + '</p><hr class="title-sep">';
  ...
  // The URL is a live link (the info exports point at the info page; the
  // reader's at the reader) — href and text carry the same escaped string,
  // so a "&tab=works" query survives as &amp; in the attribute.
  h +=
    '<p class="url"><a href="' +
    escapeHTML(siteURL) +
    '">' +
...
function tocEntries(cfg) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = cfg.hasRowNums ? 1 : 0; j < row.length; j++) {
      ...
      if (!v) continue;
      var hdr =
        cfg.headerRow && cfg.headerRow[j] ? cfg.headerRow[j].toLowerCase() : "";
      if (
        hdr.indexOf("head") === 0 ||
        hdr.indexOf("kitab") === 0 ||
        hdr.indexOf("bab") === 0
...
function tocPageHTML(cfg, tocTitle) {
  ...
  var h =
    '<div class="toc"><h2>' + (tocTitle || "Table of Contents") + "</h2><ol>";
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = cfg.hasRowNums ? 1 : 0; j < row.length; j++) {
      ...
      if (row[j] && String(row[j]).trim())
        fields.push({
          value: String(row[j]).trim().replace(/\n+/g, "<br>"),
          index: j,
        });
    }
    for (var j = 0; j < fields.length; j++) {
      var colHeader3 =
        cfg.headerRow && cfg.headerRow[fields[j].index]
          ? cfg.headerRow[fields[j].index].toLowerCase()
          : "";
      if (j > 0) {
        var prevHdr3 =
          cfg.headerRow && cfg.headerRow[fields[j - 1].index]
            ? cfg.headerRow[fields[j - 1].index].toLowerCase()
            : "";
        if (prevHdr3.endsWith("ar") && colHeader3.endsWith("dv"))
          content += "<p>&nbsp;</p>";
        if (prevHdr3.startsWith("matn") && colHeader3.startsWith("sharh"))
          content +=
            '<p style="text-align:center;color:#bbb;margin:6px 0;font-size:8pt;letter-spacing:3px">· · ·</p>';
      }
      if (colHeader3.startsWith("foot") && fields.length > 1)
        content +=
      ...
      if (j > 0) {
        var prevHdr2 =
          cfg.headerRow && cfg.headerRow[fields[j - 1].index]
            ? cfg.headerRow[fields[j - 1].index].toLowerCase()
            : "";
        if (prevHdr2.endsWith("ar") && colHeader2.endsWith("dv"))
          pdfHTML += "<p>&nbsp;</p>";
        if (prevHdr2.startsWith("matn") && colHeader2.startsWith("sharh"))
          pdfHTML +=
            '<p style="text-align:center;color:#bbb;margin:6px 0;font-size:8pt;letter-spacing:3px">· · ·</p>';
      }
      if (colHeader2.startsWith("foot") && fields.length > 1)
        pdfHTML +=
          '<p style="color:#999;font-size:11pt">ــــــــــــــــــــــــــــــــــــــــــــ</p>';
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    for (var j = cfg.hasRowNums ? 1 : 0; j < row.length; j++) {
      if (row[j] != null && String(row[j]).trim()) {
        var expHdr =
          cfg.headerRow && cfg.headerRow[j]
            ? cfg.headerRow[j].toLowerCase()
            : "";
        // Line breaks in multi-line cells (the modal's fact strip, bios).
        var expVal = String(row[j]).trim().replace(/\n+/g, "<br>");
        if (expPrevHdr.startsWith("matn") && expHdr.startsWith("sharh"))
          htmlExport += '<div class="ms-sep">· · ·</div>';
...
export function initExports(ctx) {
  ...
  function setExportBusy(on) {
    if (on) {
      ...
      if (btnExport.dataset.origText)
        btnExport.textContent = btnExport.dataset.origText;
      ...
      if (fmt === "txt") {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          for (var j = exportStart0; j < row.length; j++) {
            if (row[j] && String(row[j]).trim()) {
              var mdHdr =
                ctx.headerRow && ctx.headerRow[j]
                  ? ctx.headerRow[j].toLowerCase()
                  : "";
              if (mdPrevHdr.startsWith("matn") && mdHdr.startsWith("sharh"))
                content += "· · ·\n\n";
        ...
        if (!win) {
          window.showErrorToast("PDF export failed — popup blocked");
          setExportBusy(false);
          return;
        }
          ...
          .then(function (fontBlob) {
            ...
            reader.onload = function () {
              ...
              var svg =
                ...
                fontData +
                ') format("woff2");font-weight:300}</style></defs>' +
                '<foreignObject width="100%" height="100%">' +
                '<div xmlns="http://www.w3.org/1999/xhtml" style="font-family:Hadithmv">' +
                wrapper.innerHTML +
                "</div>" +
                "</foreignObject></svg>";
              var img = new Image();
              ...
              img.onerror = function () {
                window.showErrorToast("PNG export failed");
                setExportBusy(false);
              };
        ...
        import("./export-xlsx.js")
          ...
          .catch(function () {
            window.showErrorToast("Excel export failed");
            setExportBusy(false);
          });
        return;
      } else if (fmt === "epub") {
        exportEPUB(ctx, siteURL, versionText)
          ...
          .catch(function () {
            window.showErrorToast("EPUB export failed");
            setExportBusy(false);
          });
        return;
      } else if (fmt === "yaml") {
        ...
        downloadFile(
          buildHtmlBook(ctx, siteURL, versionText, ctx.t("infoToc")),
          baseName + ".html",
          "text/html",
        );
        ...
        for (var ri = 0; ri < rows.length; ri++) {
          ...
          for (var cj = 0; cj < rows[ri].length; cj++) {
            var cv =
              rows[ri][cj] != null
                ? String(rows[ri][cj]).trim().replace(/\n+/g, "<br>")
                : "";
