@echo off
rem Hadithmv - rebuild data/search-index.json (the manifest) + the per-book
rem search shards (data/search-index/) from the book registries + CSVs.
...
rem Output: data/search-index.json (manifest) + data/search-index/<book>.json
