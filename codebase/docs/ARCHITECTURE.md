...
| `data/search-index.json`                                | Generated search-index **manifest** — meta only (bookIds, per-book shard hashes, ~2 KB); the scope picker and scoped loads read just this (see "Library search")                                                        |
...
- **Deliberately excluded**: book CSVs, the search-index manifest
  (`data/search-index.json`) and the per-book shards (`data/search-index/`)
