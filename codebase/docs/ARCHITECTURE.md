...
- **Deliberately excluded**: book CSVs, the search-index manifest
  ...
  registry hash, `loadIndexMeta`/`loadScopedIndex` on `meta.version` and the
  per-shard hashes, with an offline fallback to the stored copies). `src/`
  dev URLs are outside the SW's interest entirely.
