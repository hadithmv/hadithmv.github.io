...
the CE span the AH→CE approximation of the periods grid):

```
Desktop — the grid, thead over rows, columns aligned by construction:
┌──────────────────────────┬─────────────────────────┬──────────┬──────────────┬────────────┬──────┬────┐
│ Name                     │ Arabic                  │ Century  │ Years        │ Miladi     │ ފޮތް │ ✓  │
├──────────────────────────┼─────────────────────────┼──────────┼──────────────┼────────────┼──────┼────┤
│ މާލިކު ބިން އަނަސް          │ مالِكُ بْنُ أَنَسِ المَدَنِيُّ │ Century 2 │ (93–179 ހ.) │ (712–795 CE) │ 1    │ ✓  │
└──────────────────────────┴─────────────────────────┴──────────┴──────────────┴────────────┴──────┴────┘

Mobile (≤600px) — the strip folds away, the same row re-flows into three
flowing lines (the wrappers are display:contents on desktop, so the grid
above is untouched there); the joins are a bare · with margins (a " · "
string would lose its leading space to inline white-space collapsing at the
start of the cell's line), the ✓ gets the spacing without a dot:
