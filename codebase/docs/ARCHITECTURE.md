...
- **`?books=A,B` / book‑scope picker** — narrows the search to specific books (the tag chips narrow by category; the picker by book — `computeScope` intersects both). `#libScopeBtn` beside the search box opens a **modal** (built lazily on first open through the shared modal layer — `createModal("libScopeOverlay", …)` with the `.lib-scope-modal` flex‑column variant, so Escape/backdrop/✕ and the focus trap come from common.js): a filter box, one chip per type group (primary tag = the code prefix) toggling the whole family, a checkbox list of the books the index actually knows (`meta.bookIds` — ENTIRE‑BOOK‑excluded books like the RDF dictionaries never appear, the tag row's Radheef chip is a silent zero for the same reason), and a footer with the "N of M books selected" count and a "Done" button. The "↺ Reset" button leads the type‑chip row and is always visible — it is an action, not a state label; clicking it when nothing is scoped is a no‑op (an empty selection is every book, the tag‑row "All" metaphor: nothing ticked is not a restriction, and ticking one book from "everything" narrows to exactly it). URL‑synced like `?tags=`; the picker list itself is lazy — derived from `meta.bookIds` on first open (`ensureSearchableBooks`), falling back to the visible registry on index failure
