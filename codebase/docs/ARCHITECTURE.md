...
| `age` | diedAH − bornAH — blank when either is missing, a `~` carries over; muted like the CE | — | 48px (fixed) |
| `authors` | — | distinct authors with a searchable book in the bucket | 56px (fixed) |
...
├──────────────────────────┼─────────────────────────┼──────────┼──────────────┼────────────┼──────────┼──────┼────┤
│ މާލިކު ބިން އަނަސް          │ مالِكُ بْنُ أَنَسِ المَدَنِيُّ │ Century 2 │ (93–179 ހ.) │ (712–795 CE) │ 86       │ 1    │ ✓  │
└──────────────────────────┴─────────────────────────┴──────────┴──────────────┴────────────┴──────────┴──────┴────┘

The same period's row (bucket 2 — the age column's sibling feature, the
distinct-author count; here Malik's is the only book in the bucket):
┌───────────┬──────────────┬────────────┬─────────┬──────┬────┐
│ Century   │ Years        │ Miladi     │ Authors │ ފޮތް │ ✓  │
├───────────┼──────────────┼────────────┼─────────┼──────┼────┤
│ Century 2 │ (93–179 ހ.) │ (712–795 CE)│ 1       │ 1    │ ✓  │
└───────────┴──────────────┴────────────┴─────────┴──────┴────┘
...
of the cell's line), the ✓ gets the spacing without a dot. Authors: three
lines, the age carrying its ޢުމުރު label; periods: two, the authors count
leading its line unjoined:
    މާލިކު ބިން އަނަސް · مالِكُ بْنُ أَنَسِ المَدَنِيُّ
    Century 2 · (93–179 ހ.) · (712–795 CE) · ޢުމުރު: 86
    ފޮތް: 1 ✓
