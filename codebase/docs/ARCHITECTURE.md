...
| `src/css/reader-quran.css`                              | Reader: Quran nav row, dropdowns, surah overlay. **Injected by reader.js on QRN detection only** — inserted before reader-search.css so reader.css still loads last.                                                    |
...
| `src/js/quran-data.js`                                  | Quran pure data/logic: loading, merging, ayah decoration, structural derivation — detection + column classification moved to book-data.js. Lazy-loaded with quran-ui.js for QRN books                                   |
| `src/js/quran-ui.js`                                    | Quran UI: surah/ayah/juz dropdowns, content presets, display options, surah selector. Re-exports quran-data.js (barrel). Dynamically imported by reader.js for QRN books only                                           |
| `src/js/csv.js`                                         | Tiny CSV parser (~1 KB) — `parseCSV()`, `unparseCSV()`, `fetchCSVRows()` + the streaming path (`createStreamParser`, `fetchCSVStreamed` — chunk-fed, 256 KB threshold)                                                  |
| `src/js/search-utils.js`                                | Search engine: normalisation, parsing, matching, snippets, history, HTML/XML escaping                                                                                                                                   |
| `src/js/library-search-engine.js`                       | Cross-book search: scope-aware index loader (manifest + per-book shards, IDB-cached) + pure query engine — `loadIndexMeta`, `loadScopedIndex`, `searchLibrary`, `tokenizeText` (shared with the index build script)     |
...
| `data/08-rebuild-searchIndex.mjs`                       | Node build script — scans every registered book, emits the search-index manifest + per-book shards (rerun after book changes)                                                                                           |
| `data/search-index-manifest.json`                       | Generated search-index **manifest** — meta only (bookIds, per-book shard hashes, ~2 KB); the scope picker and scoped loads read just this (see "Library search")                                                        |
| `data/search-index/*.json`                              | Generated per-book search shards — one flat `{word: packed-ranges}` file per book (the loader fetches only the shards for the books in scope)                                                                           |
...
| CSV parsing                                                      | `csv.js`                                                                                                 | `parseCSV`, `fetchCSVRows`, `parseCSVWithHeader`, `fetchCSVObjects`, `createStreamParser`, `fetchCSVStreamed` — the streaming path (chunk-fed, 256 KB threshold)                                                                                                                                                                                                                                                                   |
...
| Library search                                                   | `library-search-engine.js`                                                                               | `loadIndexMeta` (manifest only — the picker), `loadScopedIndex` (manifest + scope's shards, merged into a module-scope master), `searchLibrary`, `tokenizeText` (shared with the index build script)                                                                                                                                                                                                                               |
...
Big books (response ≥ 256 KB — `fetchCSVStreamed`'s threshold in `csv.js`) are fetched and parsed **incrementally** so the first rows render before the download finishes. The stream parser (`createStreamParser`) is chunk-fed and produces rows byte-identical to `parseCSV` (same trims, `keepEmpty` rule, `""` escaping and row splitting — including multi-byte Thaana/Arabic split across chunks via a `TextDecoder({stream:true})` wrapper, a held trailing `\r` that might be half of `\r\n`, and a held trailing `"` that might be half of `""`). The reader consumes it through callbacks:

- the **header row** (first emitted row) triggers `buildReader` with the first 128-row batch, and the reader **reveals immediately** — the skeleton gives way to real rows while the download still runs (this is the whole point; the reveal must happen at the first batch, not at `finalize`, or the rows exist in the DOM behind the hidden wrapper and streaming is invisible — the regression the battery's visibility checks guard). Search/filter/export are **gated** (disabled, plus a keyboard guard on the search-window shortcut) until the stream ends; the pagination strip shows "…" until the total is known — a `streamActive` flag in the reader's pagination context keeps `updatePagination` from ever advertising a partial total or a jump input mid-stream (a far jump target would have to append the whole backlog synchronously), and the "…" stub seeded by the streaming bridge is replaced by the real strip when the drain completes;
- later batches (128 rows) arrive through a bridge that appends them to the same `allData`/`filteredData` arrays the built reader owns. Appends only **arm** a 150 ms flush timer (`append` never renders eagerly) — batches coalesce in both view modes, so a burst (see the service-worker note below) does not turn into one synchronous DOM update per batch;
- the **progress line renders INSIDE `#pageTitle` itself**: `#pageTitle` is the `flex:1` child of `#topBar` with `justify-content: safe center` — its text is *centred in its slot*, and that slot is exactly where the title appears, so the line is written as the title's content (`_streamProgress` sets `innerHTML`) and sits at the true centre — a sibling flex child would sit at the row's start instead (that misplacement is what the current design fixes). It shows "Loading… N%" from Content-Length vs bytes read (clamped — the browser reports decompressed bytes against the compressed total); the word comes from i18n (`t("loading")`, dv/en/ar), so it reads in the visitor's UI language (the streaming battery forces `lang=en` to match the text). The `.loading-progress` span **inherits the title's direction and font** — in Dhivehi/Arabic the RTL title keeps the line RTL (word right, percent left, one reading order; a latin-font override was tried and dropped: it re-ordered the line backwards *and* made the Thaana word fall back to a system face) — the English variant is the only one that overrides to LTR (`html[lang="en"] .loading-progress { direction:ltr; unicode-bidi:isolate }`, keyed off the `<html lang>`/`data-lang` i18n.js sets; the webfont covers both scripts at the title's size `--fs-title`, so the swap reads as one element changing state); the percent sits in a bidi-atomic inline-block cell (`.loading-progress-pct`, min-width 4ch, right-aligned) so "42%" stays one unit in RTL run ordering and the digits grow leftward without the % drifting. The line lives outside the skeleton (the reveal can't hide it) and outside `#readerWrapper` (the growing table never pushes it around; it used to sit inside the wrapper with a big dead gap below it). `streamFinalize` flips `isStreaming` off and re-runs `updatePageHeader` — the real title renders in place of the line **the moment the data is complete** (the bridge's `finalize` runs it before the cosmetic DOM fill — see the drain below; for a 152,612-row book the fill takes minutes, and the line must not sit at "Loading… 100%" with search blocked while it runs); while the line is up the title's own click handler is gated (`isStreaming`). A failed stream restores the stashed pre-stream title text (`showError` — `_streamProgress` stashes it on first contact). Changing the UI language mid-stream re-writes the line with the new word on the next progress tick (≤1 tick transient);
- **the drain**: when the download finishes ahead of the render, `finalize` must not run the remaining backlog in one synchronous while-loop — that would block the main thread for the whole build. Instead a tick-based drain flushes on 50 ms timers with an adaptive step (`max(stream tick, ceil(backlog/20))`), and in table mode groups up to **4 inserts per tick** so paints stay bounded (~5–6 total) — table-layout `auto` recomputes every column from all rows on each paint, which is the real cost of a burst backlog (the battery's table-mode pass and the slow-4G probe exist to catch starvation regressions: without the timer yield, a SW-cached burst can freeze the page for many seconds). The drain is **purely visual** — the title, strip total, gating and search release when the data is complete (see the line paragraph), not when the fill lands. And the fill is a **window, not the whole book**: it stops at `DRAIN_MAX_ROWS` (20,000) because every insert forces a full-document layout whose cost grows with the row count — draining the merged RDF-all's 152,612 rows wedged the page for a minute inside a single late tick. Past the cap the scroll window (sentinel appends) owns the fill, and a mid-drain search ends the drain at the filtered view's end (`drainTarget` = min of allData, filteredData, cap) instead of spinning on rows that no longer exist;
- the reader's CSV loop yields to the event loop on every chunk (`await new Promise(res => setTimeout(res, 0))`), so timer-driven UI (the progress line, the drain) always gets a turn. One empirical quirk: fetches served from the **service worker bypass DevTools/`Network.emulateNetworkConditions` throttling** (the SW delivers its cached copy at full speed), so the throttled batteries 404 `sw.js` by design and slow-4G probes have to serve over HTTP — a throttled burst arrives in ~1 s no matter the setting;
- the final row batch is delivered after `parser.finish()` — a trailing lone `\r` or a file with no trailing newline only closes at the tail, so flushing before `finish()` would orphan the last row (off-by-one, caught by the streaming battery).

Whole-file fallbacks, in order: responses under the threshold, a missing `Content-Length`, no `ReadableStream`/`TextDecoder`, and IndexedDB cache hits (repeat visits are instant and never stream). `-DSC` books display rows last-to-first, so the "first" streamed row would be the LAST visible one — they keep the whole-file path. The IDB record is identical in both paths (full array, header first; the write happens after the stream completes). One practical consequence of the `Content-Length` gate: `file://` responses never carry it (verified in headless Edge — the guard sees total 0), so local double-click previews always take the whole-file path and never show the progress line; only HTTP(S) streams.

**Quran books stream too.** QRN books (the heaviest in the library — a 10 MB translation among them) go through a merge, not a straight parse, and the merge itself streams: `mergeQuranDataStreamed` (quran-data.js) loads the small base skeleton first (see "Quran data model"), then streams the book CSV through the same `fetchCSVStreamed` machinery, merging each 128-row batch into the base skeleton **before** the reader's `onRows` sees it — a running cursor across batches keeps the row alignment exact, so batch boundaries never shift it. The reader bridge, progress line, title swap, drain and gating are all the standard path's, with one addition: the quran surah/ayah/juz nav joins the gated controls for the stream's duration (a mid-stream jump would replace the filtered view with a slice of a partial `allData` — the stream's appends only reach `allData` — freezing the reader on partial content). The promise resolves **null** when the stream engaged (the reader consumed every row through the callbacks; csv.js already stored the RAW book record in IDB — cache records stay byte-compatible with the whole-file path) or `{headerRow, allData}` from the whole-file fallback (cache hit, sub-threshold, no ReadableStream) — one promise shape either way, and both paths merge with the same base-bounded, index-aligned rule and the same console warning when a book's row count ≠ the base's 6,236 (QRN-soabuniAshari-HDN is legitimately short at 6,147 rows; rows past the base's end are dropped).

**The virtual merged radheef book (RDF-all) streams too.** It has no content CSV — radheef-merge.js assembles the combined view from the 8 source radheef books, and until now that meant waiting for all ~15 MB before a single row. `loadMergedRadheefBookStreamed` (radheef-merge.js) fixes that: a **phase 0** HEADs all 8 sources in parallel and sums the Content-Lengths — that sum is the progress line's total; a **phase 1** streams the sources *sequentially* in MERGED_SOURCES order (rasmee leads — the deliberate block order), projecting each 128-row batch into the 7-column merged schema **before** the reader's `onRows` sees it (the projection is a shared `projectBatch`, byte-identical to the whole-file assembly's). The merged header is static (MERGED_HEADERS) and emitted once, before the first source. The aggregate fraction is `(completed sources' bytes + the current source's share) / total` — monotonic, clamped at 1 for compressed-vs-decompressed drift, same as the single-book line. Cache-hit / sub-threshold sources arrive whole-file through the same bridge; failed sources are skipped with their bytes advanced (the whole-file rule). Any phase-0 HEAD failure (network, no Content-Length, `file://`) keeps the whole-file assembly — one promise shape either way (`null` when the stream engaged, `{data, headerRow, hasRowNums}` otherwise). The reader wiring, progress line, title swap, drain, gating and `-DSC`/IDB rules are all the standard path's — only the loader differs.
...
Every translation CSV must have the same number of rows, in the same ayah order, as the base file — row _N_ of the translation merges into row _N_ of the reader. An untranslated ayah is an **all-empty row** (`,,,` or a blank line) — leave it in place; it renders as the base columns with no translation. `mergeQuranData` (whole-file) and its streaming twin `mergeQuranDataStreamed` (used by the reader — see "Big-book streaming") log a console warning when a book's row count differs from the base's 6,236: a structural mistake in the CSV (rows added/removed, or a trailing newline parsing as an extra slot).

**Column loading.** `loadQuranBookCSV()` keeps a one‑entry parse cache (most recent book only): each translation CSV is fetched and parsed at most once per session, so inserting several columns from the same book — or a preset hitting multiple books — does not re‑download or re‑parse the whole file per column. The registry groups each book's columns together, so consecutive inserts hit the cache. On‑demand loads stream (`loadQuranBookCSV` accepts `streamOpts`, passed to `fetchCSVStreamed`) and carry a status footer pinned to the modal's bottom edge — the interface language's `t("loading")` + the loading column's display label (the registry `displayDV`/`displayEN` — the modal row's own name, never the ASCII book code) + N%, with a **Cancel** button that aborts the fetch via `AbortSignal` (csv.js `fetchCSVStreamed` + the `loadAndInsertColumn` settle check). The footer is out of flow (`position: absolute` in the modal body): its show/hide never shifts the column list, so the cursor stays on the row the user just clicked (an earlier in‑flow line above the table shoved the whole list down at every load start/end); the list under it is fully gated while a load runs, and the bar passes the wheel through to the list. A cancelled download settles silently: no toast, no column, and no IDB record (the cache write happens only after the fetch resolves). Presets load **sequentially** — one download at a time: while a load is in flight the gate disables every modal checkbox and preset button (`_loadInProgress` in `quran-ui.js`), so concurrent downloads are impossible by construction. Cancelling a preset stops after the current book; columns that already landed stay. Cancellation is decided **per target, by the AbortController that target's load created** — `loadAndInsertColumn` assigns a fresh controller for every invocation (instant targets included), and the preset queue's settle tests that controller, never a global past-tense. (An earlier version left the controller of a cancelled load in place; the next preset then read "cancelled" before its first target and silently dropped every download — the preset buttons "dead" until reload.) **Rebuilds defer while the modal is open** (`scheduleReaderRebuild` marks `_readerDirty`; closing the modal — every path — flushes one keep-spot rebuild, `quran-ui.js` + the `rebuildAllKeepSpot` wrapper on reader.js's quranCtx). A column insert adds text to every row in the rebuild window above the viewport, so a live rebuild must slide the page down by that growth to keep the reading row on screen — the preset buttons used to visibly scroll the page behind the modal by thousands of px per landing. Deferred to the close, the flush's scroll adjustment cancels the growth exactly: nothing moves, and N landed columns cost one rebuild. A settle that lands after the modal already closed (a preset left downloading) rebuilds immediately — the reader is live again.

**Column ordering.** The content modal (unified `createModal` layer) shows a reorderable table — checkboxes and ▲▼ buttons per row, presets fixed above a scrollable list. `_colOrder` (registry order by default) is the single source of truth for reader column order: `applyColumnOrder()` (pure, in `quran-data.js`) rebuilds header, rows, the norm cache, and hidden indices from it, so loaded columns appear in the list's order rather than insertion order. Base columns are fixed first; moving a loaded column reorders the reader when the modal closes — the reorder buttons live inside the overlay, so their rebuilds defer like every other modal-driven change (see "Column loading"). The `-1` marker in the loaded map tags a pending insert until the rebuild assigns its real index.
...
- **Content modal**: checkboxes + ▲▼ reorder for all columns from the registry; changes apply when the modal closes (reader rebuilds defer while it is open — see "Column ordering" under Merging)
...
**Font.** A single merged WOFF2 font covers Arabic, Thaana, and Latin glyphs: the full original at `src/font/merged-400.woff2` (loaded by the unminified source pages), the corpus subset at `dist/font/merged-400.woff2` (served by the optimized tree). It is a **subset** carved by `tools/hmv-font-subset.py` (Python 3 + fonttools — one-time `pip install fonttools brotli`): the tool scans the corpus — `data/**/*.csv` + `data/**/*.json`, `static/notes/**/*.md`, `src/books/*.html`, `src/js/*.js`, `src/css/*.css`, HTML-entity- and JS-`\u`-escape-decoded — and rebuilds the font to exactly those codepoints (currently 648 corpus codepoints across 193 files → 323 kept, 754 glyphs; 97,320 B woff2 + 112,832 B woff, down from 167,760/199,084). GSUB/GPOS features are kept whole (Arabic positional forms, lam-alef ligature, `ccmp`, `locl` — shaping is intact). The full font is archived, committed once, at `src/font/` (the ingredient drawer) and never overwritten — every carve is made from it and writes `dist/font/` (the woff built from the same frozen bytes as the woff2). `dist-build.mjs` wipes dist/ each build and then runs the carve, so the served tree always ships a fresh subset; a standalone tool run handles content-only changes. Output is byte-stable across runs (the `head.modified` stamp is suppressed). The tool verifies coverage after writing and refuses to re-subset an already-carved font (if `src/font/` ever holds a subset too, `git checkout HEAD -- src/font/merged-400.*` restores it). Corpus characters the source font never covered — 326 today: control codes, Quranic marks (ۖ…), transliteration letters (ḍ, ḥ, ṭ…), emoji — fall back to system fonts exactly as before; the tool lists them and fails only on a _regression_ (a character the source covered that the subset lost). After any change that introduces characters, run `python tools/hmv-font-subset.py --check` (or the full run) — see TESTING.md. A full run also writes `font-build-report.md` (codebase root) — a machine-generated, committed, diffable ledger of each carve (corpus stats, per-flavor before→after sizes, coverage, sha256), in the same pattern as `dist-build-report.md` / `data/search-index-report.md`.

**Why no `unicode-range` split.** Content is trilingual by design and independent of the UI language: every page — reader, library, search, info — mixes Arabic matn, Dhivehi translation, and Latin digits/UI regardless of the UI language, so a split would download all three subfonts on every session anyway: three requests, duplicated shared glyphs (digits, punctuation), and one more file to cache — strictly worse than one carved file. The merged subset is the only real font win. The per-content win lives elsewhere: **per-export subsetting** — `export-epub.js` embeds the whole webfont in every EPUB; a future carve-by-book (embed only the exported book's own corpus union) would shrink embeds substantially. Noted here for that future work.

`font-family` stacks always list `"Hadithmv"` first, then platform fallbacks. Never load external fonts. Each family is a `--font-*` var in `:root` (common.css) — use sites are `var(--font-*, <canonical>)`, and the only literal `font-family` in the stylesheets is the `@font-face` name. Families: `--font-mixed` `"Hadithmv", "Faruma", system-ui, -apple-system, sans-serif` (any-language controls); `--font-latin` `system-ui, -apple-system, sans-serif` (Latin-only chrome); `--font-arabic` `"Hadithmv", "Traditional Arabic", "Scheherazade New", serif` (Arabic-only content); `--font-arabic-thaana` `"Hadithmv", "Traditional Arabic", "Scheherazade New", "Faruma", serif` (mixed Arabic/Thaana content); `--font-thaana` `"Hadithmv", "Faruma", "MV Boli", sans-serif` (Dhivehi titles); `--font-mono` `"Consolas", "DejaVu Sans Mono", "Courier New", monospace`. (export.js / export-epub.js embed literal stacks in generated SVG/EPUB documents — standalone files, no vars there.) Title lines: Arabic and Dhivehi share one size on every surface (cards and search results) — `calc(var(--panel-font-size[-mobile]) * var(--title-scale))`, `--title-scale: 1.2` in `:root` — while English titles stay at base; hierarchy is carried by size tier, weight, and colour. Title lines sit `--title-gap` (4px) apart, with `--title-gap-caption` (2× the base gap) before the English caption; on cards the whole title block sits at the top, with the space below coming from the card's padding.
...
| `_baseDataCache` / `_surahNamesCache`                                            | `quran-data.js` module scope            | first load only                                                                               |
| `_columnSourceMap` / `_colRegistryCache`                                         | `book-data.js` module scope             | QRN column source map + registry (moved with the classification helpers)                      |
| `_metaPromise` / `_dbPromise`                                                    | `library-search-engine.js` module scope | first `loadIndexMeta()` call; cleared on failure so retries work                              |
| `_shardPromises` / `_loadedShards` / `_mergedVersions` / `_master`               | `library-search-engine.js` module scope | per-`code:version` shard loads; `_master` grows monotonically as scopes widen                 |
...
- **Font coverage**: after any change that adds characters (new book, notes, i18n keys, export writers, HTML/CSS content), `python tools/hmv-font-subset.py --check` must pass — the corpus-vs-subset diff, see TESTING.md (one-time `pip install fonttools brotli`; the tool doubles as the subsetter itself)
...
- `src/css/*.css` → `dist/css/`, minified **in place** by **lightningcss**
  ...
  lightningcss"). `url()` paths are untouched, so the shared `../font/...`
  keeps its depth (resolving to `src/font/` from the source css and
  `dist/font/` from the built css)
...
- **Deliberately excluded**: book CSVs, the search-index manifest
  (`data/search-index-manifest.json`) and the per-book shards (`data/search-index/`)
  are not in the manifest and pass straight through — the app's own
  IndexedDB caches own them (`fetchBookCSVCached` version-gates on the
  registry hash and streams big books on first visit, `loadIndexMeta`/
  `loadScopedIndex` on `meta.version` and the per-shard hashes, with an
  offline fallback to the stored copies). `src/` dev URLs are outside the
  SW's interest entirely.
...
| Big-book streaming   | API.md `fetchCSVStreamed`                 | ≥256 KB responses parse incrementally — first rows + a progress line while downloading; search gated     | first rows ~1 s; 100% across ~17 s at 500 KB/s        |
| Search-index caching | API.md `loadIndexMeta` + Library search   | manifest-only loads (304 + IndexedDB); per-shard fetches `?v=`-busted, only for the books in scope       | 16.5 MiB → 1.9 KB gz; fetches = scope only            |
...
`data/08-rebuild-searchIndex.mjs` (Node — run `node data/08-rebuild-searchIndex.mjs` after book changes, or double-click `codebase/rebuild-index.bat`; chain it after the PS1) scans every registered book once, offline, and emits a **manifest** (`data/search-index-manifest.json` — meta only, ~2 KB gz) plus **one shard per indexed book** (`data/search-index/<bookCode>.json`). The postings live in the shards — word‑level `bookId + row` entries, where `bookId` is a numeric index into `meta.bookIds` (full codes never repeat per entry). Built with the app's own parser and normaliser (`parseCSV`, `normaliseForSearch`) and the SAME tokeniser the query side uses (`tokenizeText` in library-search-engine.js — build and query MUST agree on what a word is, so the script imports it rather than re‑implementing). `-HDN` columns and the row‑number column are excluded; an optional `excludeFromIndex` registry column (comma‑separated header names) skips those columns — `-HDN` and the row‑number column still win regardless; the magic value `ENTIRE-BOOK` skips the whole book (no postings, and it is listed under `## Excluded Books` in the report). The build prints **one report line per book** (row count, indexed columns, skipped columns) and writes the same info to `data/search-index-report.md` (markdown table with per-book postings — the policy as a diffable file, committed alongside the index) so the whole indexing policy is eyeballable at a glance, and an `excludeFromIndex` entry that matches no column warns. The build times itself — elapsed, per‑phase breakdown (index / pack / write), rows·postings per second, heap, node version — printed to the console and mirrored in the report's `## Build Stats` section. Rows are packed as ranges (`"1-5,8,12"`); pure‑number tokens are dropped. `meta.version` is the SHA‑256 of everything except `built` (first 16 hex — a no‑op rebuild is version‑stable, proven by the build's two‑run hash‑compare gate); each shard's 16‑hex SHA‑256 sits in `meta.shards` and cache‑busts its fetch (`?v=<hash>` — the URL changes exactly when the shard changes, so Pages' heuristic caching can never serve stale postings while the manifest flips). Shards are written first, the manifest last — a new manifest never references a missing shard — and the build asserts the `shards` keys equal `bookIds` in both directions, then sweeps stale shard files so a removed book can't rot in the repo. **Row numbers are 1‑based DATA POSITIONS** (the reader's `?row=` contract — `goTo(row-1)`) — NOT the CSV's `#` column, which is not always sequential (5 books have gaps); the index would deep-link to the wrong row otherwise. The `#` column is display-only.

The manifest's shape — real excerpt (`bookIds` truncated):

```json
{
  "meta": {
    "version": "f916e5a48a96f6e7",
    "built": "2026-08-30T06:28:46.263Z",
    "bookIds": ["AQD-aqidatuRaziyain_ASM", "…"],
    "books": 65,
    "rows": 44775,
    "words": 206510,
    "shards": { "AQD-aqidatuRaziyain_ASM": "9320f18ff7ac9ac9", "…": "…" }
  }
}
```

and one shard — `data/search-index/AQD-aqidatuRaziyain_ASM.json` (a shard IS one book, so its postings carry no book key):
...
Once merged, posting keys are indices into `meta.bookIds` — `"0": "1-3"` reads "data positions 1–3 of `meta.bookIds[0]`" (`AQD-aqidatuRaziyain_ASM`). `rows` counts scanned rows across books; `words` counts unique tokens.

Current size: 65 books, 44,775 rows, 206,510 unique words — the manifest is 4.8 KB raw / 1.9 KB gzipped; the 65 shards total 19.4 MB raw / 6.0 MB gzipped (median 27 KB gz; the largest, the Quran books, up to ~1.1 MB gz). GitHub Pages on‑the‑fly gzips JSON for clients that accept it: a browser's `fetch()` (which always sends `Accept-Encoding: gzip, deflate, br`) receives `Content-Encoding: gzip`, decompressed transparently by the browser — so the loader's `resp.text()` is unaffected. (Probing with `curl -I` without an `Accept-Encoding` header shows the raw size and no `Content-Encoding`; that's a client‑side header, not a server config.) A scoped search downloads only its scope's shards — the page's default scope (the HDT group — 16 books) is a few tens of KB on a cold session, the picker alone costs 1.9 KB, and an all‑books search pays the full 6.0 MB once (then it lives in IndexedDB forever). Shards re‑download only when their `?v=` hash changes. Whole‑word matching only — substring, fuzzy, and regex stay in‑book (see "What's deliberately different" below).

### The loader

`src/js/library-search-engine.js` — a pure module (no DOM). Loading is two‑stage and scope‑aware. `loadIndexMeta()` fetches the manifest only — a conditional request (`cache: "no-cache"` → a cheap 304 when unchanged; ~2 KB to parse) — memoized and cleared on failure; this is the scope picker's whole dependency. `loadScopedIndex(scopeBookCodes)` loads the manifest, then the shards for exactly the books in scope (null / absent / empty = every indexed book; unknown codes are dropped so a garbage deep link never 404‑fetches), merging them into a module‑scope master dict — the same `{word: {bookId: packed}}` shape the query engine consumes. Each shard fetches as `data/search-index/<code>.json?v=<shardHash>` — the URL changes exactly when the content changes — is memoized per `code:version` (cleared on failure) and written fire‑and‑forget to the on‑device IndexedDB copy (`hadithmvSearch` DB — deliberately separate from the book cache in csv.js so the two modules never contend on a version bump); already‑loaded shards are reused as scopes widen. **Any needed shard failing rejects the whole load** — result counts stay truthful — and the page's error + Retry re‑attempts just the missing pieces. **Offline rule**: search works offline when the manifest + all needed shards are in IndexedDB; a missing needed shard + fetch failure → the standard error + Retry, never partial results. `loadSearchIndex()` is a thin alias of `loadScopedIndex(null)` — pre‑shard callers keep working unchanged. `searchLibrary(index, query, scopeBookCodes)` normalises + tokenises the query (same `tokenizeText` as the build), looks up each word's postings, ANDs them at row level, intersects with the scope (book codes — the page passes visible books ∩ tag chips), and returns per‑book `{bookCode, count, firstRow}` sorted by match count.

### The UI

The page (module `src/js/library-search-page.js`, styles `src/css/library-search.css`) reads `?q=` and `?tags=` from the URL (shareable links — typing, chip toggles, and clear keep the address bar in sync via `replaceState`). Tag chips and the book-scope picker share one scope — a chip click sets the picker to the tag's group (`?tags=` restore does the same), and the All chip widens to every book. Fresh visits default to the HDT group (`initScopePicker` `defaultTag: "HDT"`) — the URL sync omits `?books=` while the selection is the default or exactly the chips' groups. The reader's search window inits the picker with the same default (no page callback — `initSearchWindow`'s reader branch), so its All-books tab also starts scoped to the HDT group. The default's apply notifies like any scope change, and the scope always mirrors into every surface: the page button and the window's summary share one label builder, the chips row shows the active group's chip (the All chip turns off — never "All tags" next to "16 books"), and when the selection is exactly one tag group the scope modal leads with it (its chip first in the rail, its books first in the list; unions and single books keep the palette order). A book is searched if it carries any selected tag (OR); `-HDN` books are excluded from scopes; a scope that matches no books renders "No results" rather than falling through to an unscoped search (the engine treats `[]` as "every book").
