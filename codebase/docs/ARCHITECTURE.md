...
- **`?books=A,B` / book‑scope picker** — narrows the search to specific books (the tag chips narrow by category; the picker by book — `computeScope` intersects both). `#libScopeBtn` beside the search box opens the `#libScopePopover`: a filter box, one chip per type group (primary tag = the code prefix) toggling the whole family, a checkbox list of the books the index actually knows (`meta.bookIds` — ENTIRE‑BOOK‑excluded books like the RDF dictionaries never appear, the tag row's Radheef chip is a silent zero for the same reason), and a "N of M books selected" footer. Empty selection = every book, the tag‑row "All" metaphor: nothing ticked is not a restriction, and ticking one book from "everything" narrows to exactly it. URL‑synced like `?tags=`; the picker list itself is lazy — derived from `meta.bookIds` on first open (`ensureSearchableBooks`), falling back to the visible registry on index failure
