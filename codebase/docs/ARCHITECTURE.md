...
**JS‑set tokens** have no `:root` value — JS writes them before paint (via `setProperty` or inline styles) and CSS consumes them with fallbacks: `--reader-font-size`/`--reader-font-size-mobile` (JS‑only; `common.js`) and `--panel-font-size`/`--panel-font-size-mobile` (`:root` defaults, overridden by `common.js` at runtime — see Font scaling), `--content-width` (reader content width, `common.js`, removed on reset), `--rdf-header-top` (sticky reader header offset, `reader.js`), and per‑chip `--tag-bg`/`--tag-color` (tag chip colours — inline golden‑ratio HSL styles from `book-data.js`, consumed with nested var fallbacks). These never live in the theme blocks; removing one means removing its JS setter too.
