...
One display line for a book registry entry's authors, in the current language — `"al-Bukhari (– 256 AH)"` — comma‑joined for multi-author books. `""` when the book has no author (`authorCode` empty or unresolvable). Drives the library result cards, dashboard cards, and the reader header author span — where the line renders as `" · <name>"` (the same dotted join the app's other text lines use) with the name linked to `index.html?authors=<codes>` (the filtered-dashboard jump, same homepage convention as the tag badges).
