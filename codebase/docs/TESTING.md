...
- **Security-audit claims must be re-anchored to the working tree.** Audit
  ...
  design**: "unescaped cells" is the content format (ZKR `<br>`, RDF
  `<span>`/entities), not a defect — the trust boundary is the repo, so the
