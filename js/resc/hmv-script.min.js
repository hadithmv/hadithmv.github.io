...
function toggleQuranFili() {
  var a = $(".dataTable").DataTable().column(4).visible();
  $(".dataTable").DataTable().column(4).visible(!a);
  a = $(".dataTable").DataTable().column(5).visible();
  $(".dataTable").DataTable().column(5).visible(!a);
}
function uthmaniImlai() {
  var a = $(".dataTable").DataTable().column(4).visible();
  $(".dataTable").DataTable().column(4).visible(!a);
  a = $(".dataTable").DataTable().column(6).visible();
  $(".dataTable").DataTable().column(6).visible(!a);
}
function filiString() {
...
function rasmString() {
  var a = document.getElementById("toggleUthmaniImlai");
  "&nbsp; ރަސްމު އުޘްމާނީ &nbsp;" === a.innerHTML.trim()
    ? (a.innerHTML = "&nbsp; ރަސްމު އިމްލާއީ &nbsp;")
    : (a.innerHTML = "&nbsp; ރަސްމު އުޘްމާނީ &nbsp;");
}
...
function loadFont() {
  bangersFont
    ...
    .catch(function (a) {
      console.log("Failed to load font: " + a);
    });
}
function changeBookQuran(a) {
