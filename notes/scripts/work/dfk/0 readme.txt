...
9 !!! open json content in vscode
save as input.json
replace "#" with "\n"
(without quotes, and make sure regex is OFF)

10 convert to csv
use either powershell, or js papaparse unparse, or below:
	/ https://www.convertcsv.com/json-to-csv.ht, dont include header in first row
...
8 import to google sheets. you now have the footnotes column that correctly matches body. copy the footnotes column AS IS (not values only) to the main table.
	[is this even needed now?] remove bottom empty rows, trim whitespace

9 remove extra bottom footnotes in last body text cell or last footnotes cell if they exist
...
