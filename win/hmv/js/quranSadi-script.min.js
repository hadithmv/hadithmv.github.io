$(document).ready(() => {
  ...
  for (var d = 0; d < quran_DB.length; d++)
    if ("" === quran_DB[d][0]) {
      let a = d + 1;
      for (; a < quran_DB.length && "" === quran_DB[a][0]; ) a++;
      a < quran_DB.length && (quran_DB[d][0] = quran_DB[a][0]);
    }
  d = (function (a, b) {
    return Array.from({ length: Math.max(a.length, b.length) }, (c, e) =>
      [].concat(
        Array.isArray(a[e]) ? a[e] : Array(b[e].length).fill(""),
        Array.isArray(b[e]) ? b[e] : Array(a[e].length).fill("")
      )
    );
  })(surah_juz_basmalah_ayah_uthmani_DB, quran_DB);
  const f = $("#quranTable").DataTable({
    data: d,
    columns: [
      {
        data: 0,
    ...
    buttons: [
      {
        ...
        text: "ކޮޕީ",
        messageTop: "تفسير السعدي - تيسير الكريم الرحمن في تفسير كلام المنان",
        title: "",
        customize(a) {
          a = a.replace(/\r\n|\n/g, "\t");
          ...
          a = a.replace(/ބިސްމި\t/g, "");
          a = a.replace(/ގުރްއާން އަރަބިން\t/g, "");
          a = a.replace(/ގުރްއާން ފިލިނުޖަހާ\t/g, "");
          a = a.replace(/ރަސްމު އުޘްމާނީ\t/g, "");
          a = a.replace(/ތަފުސީރު\t/g, "");
          a = a.replace(/ތަފުސީރު ފިލިނުޖަހާ\t/g, "");
          a = a.replace(
            /‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾\t/g,
            "\n\n‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾\n"
          );
  ...
  document.addEventListener(
    ...
    function (a) {
      if (g && h) {
        var b = g - a.touches[0].clientX;
        Math.abs(b) > Math.abs(h - a.touches[0].clientY) &&
          (0 < b
            ? f.page("previous").draw("page")
            : f.page("next").draw("page"));
        h = g = null;
      }
    },
    !1
  ...
  $(".dataTable").on("dblclick", "tr", function () {
    "" !== f.search() && f.search("").draw();
    f.row(this).draw().show().select().draw(!1);
  });
